...
public class Rule_06_P3 : Rule_P
{
    ...
    public void Initial()
    {
        ...
        IsInBoxed = false;
        StayTime = 0;
        print("Rule_06_P3_Initial");
    }
    private void OnTriggerStay(Collider collision)
    {
        if (collision.gameObject.tag == "DroveCore" && (Rule_06.NowState == 1))
        {
    ...
    void Update()
    {
        if (Rule_06.NowState == 1)
        {
            Rule_06.NowInBox = this.NowInBox;
            DetectClosed();
        }
        if (IsInBoxed && NowInBox == false && (Rule_06.NowState == 1))
        {
            StayTime = 0;
            print("error06_P3");
        }
        if (StayTime > 5 && (Rule_06.NowState == 1))
        {
            Rule_06.SwitchBool = true;
            Rule_06.NowState = 2;
            print("From 06_P3 State 1 successed");
            this.Initial();
