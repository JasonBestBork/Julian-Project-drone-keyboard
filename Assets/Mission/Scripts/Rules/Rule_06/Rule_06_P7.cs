...
public class Rule_06_P7 : Rule_P
{
    ...
    public void Initial()
    {
        ...
        IsInBoxed = false;
        StayTime = 0;
        print("Rule_06_P7_Initial");
    }
    private void OnTriggerStay(Collider collision)
    {
        if (collision.gameObject.tag == "DroveCore" && (Rule_06.NowState == 4 ))
        {
    ...
    void Update()
    {
        if (Rule_06.NowState == 4)
        {
            Rule_06.NowInBox = this.NowInBox;
            DetectClosed();
        }
        if (IsInBoxed && NowInBox == false && (Rule_06.NowState == 4))
        {
            StayTime = 0;
            print("error06_P7");
        }
        if (StayTime > 5  && (Rule_06.NowState == 4))
        {
            Rule_06.SwitchBool = true;
            Rule_06.NowState = 5;
            print("From 06_P7 State 4 successed");
            this.Initial();
