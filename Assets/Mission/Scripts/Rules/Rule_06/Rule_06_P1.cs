...
public class Rule_06_P1 : Rule_P
{
    ...
    public void Initial()
    {
        ...
        IsInBoxed = false;
        StayTime = 0;
        print("Rule_06_P1_Initial");
    }
    private void OnTriggerStay(Collider collision)
    {
        if (collision.gameObject.tag == "DroveCore" && (Rule_06.NowState == 2))
        {
    ...
    void Update()
    {
        if (Rule_06.NowState == 2 )
        {
            Rule_06.NowInBox = this.NowInBox;
            DetectClosed();
        }
        if (IsInBoxed && NowInBox == false && (Rule_06.NowState == 2))
        {
            StayTime = 0;
            print("error06_P1");
        }
        if (StayTime > 5 && (Rule_06.NowState == 2))
        {
            Rule_06.SwitchBool = true;
            Rule_06.NowState = 3;
            print("From 06_P1 State 2 successed");
            this.Initial();
