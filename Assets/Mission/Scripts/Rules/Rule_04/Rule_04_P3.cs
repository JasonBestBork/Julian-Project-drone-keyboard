...
public class Rule_04_P3 : Rule_P
{
    ...
    {
        if (collision.gameObject.tag == "DroveCore" && (Rule_04.NowState == 4))
        {
            Debug.Log("Exit_test");
            NowInBox = false;
        }
    ...
    void Update()
    {
        if ((Rule_04.NowState == 4))
        {
            Rule_04.NowInBox = this.NowInBox;
            DetectClosed();
        }
        if (IsInBoxed && (Rule_04.NowState == 4))
        {
            Rule_04.SwitchBool = true;
            Rule_04.NowState = 5;
            print("From 04_P3 State 4 successed");
            this.OnEnable();
