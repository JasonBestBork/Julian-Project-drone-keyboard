...
public class Rule_01_P1 : Rule_P
{
    ...
    {
        if (collision.gameObject.tag == "DroveCore" && (Rule_01.NowState == 1))
        {
    ...
    void Update()
    {
        if(Rule_01.NowState == 1)
        {
            Rule_01.NowInBox = this.NowInBox;
            DetectClosed();
        }
        if (IsInBoxed && NowInBox == false && (Rule_01.NowState == 1))
        {
            StayTime = 0;
            print("error01_P1");
        }

        if (StayTime > 5 && Rule_01.NowState == 1)
        {
            Rule_01.SwitchBool = true;
            Rule_01.NowState = 2;
            print("From 01_P1 State 1 successed");
            this.Initial();
